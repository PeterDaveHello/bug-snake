...
async function createHarness(fetchImpl, { failCacheOpen = false } = {}) {
  ...
  return {
    ...
    async dispatchActivate() {
      ...
      if (!activationPromise) throw new Error('Activate handler did not call waitUntil');
...
{
  const harness = await createHarness(async () => new FakeResponse('network'));
  for (const cacheName of [
    ...
    await harness.caches.open(cacheName);
  }

  await harness.dispatchActivate();
  const cacheNames = await harness.caches.keys();
  assert(
    !cacheNames.includes('bug-snake-v0') && !cacheNames.includes('bug-snake-v1'),
    'Activate removes older versioned bug-snake caches'
  );
  ...
  assert(
    cacheNames.includes('another-app-v1') && cacheNames.includes('some-other-project-cache'),
    'Activate preserves caches owned by other applications on the same origin'
  );
  assert(harness.getClaimCalls() === 1, 'Activate claims clients after scoped cache cleanup');
}
