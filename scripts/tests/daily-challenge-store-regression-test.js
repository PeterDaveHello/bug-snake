...
function runDailyStoreRegressionTests() {
  ...
  const quotaResult = new DailyChallengeStore(newDayQuota.storage).recordResult(incomingChallenge, 777);
  const reloadedQuotaRecord = new DailyChallengeStore(newDayQuota.storage).getRecord(incomingChallenge);
  ...
  const partialReload = new DailyChallengeStore(partialRemovalQuota.storage).getRecord(partialChallenge);
  assert(rejectedFirstRemoval, 'Quota recovery regression test exercises an individual deletion failure');
  assert(
  ...
  assert(fallbackRecord.bestScore === 555, 'Unreclaimable quota still keeps the result for this session');
  assert(
